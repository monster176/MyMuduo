            ...
            , highWaterMark_(64 * 1024 * 1024)  // 64 M
{
    ...
    LogINFO("TcpConnection::ctor[%s] at fd = %d \n",name_.c_str(),sockfd);
    socket_->setKeepAlive(true);    //  TCP 的 保活机制     
}

TcpConnection::~TcpConnection()
{
    LogINFO("TcpConnection::dtor[%s] at fd = %d state = %d \n",\
...
{
    LogDEBUG("TcpConnection::handleWrite() ===========> IN");
    if(channel_->isWriting())
    {
        ...
        if(n > 0)
        {
            ...
            LogERROR("Tcp Connection ::handleWrite");
        }
    }else{
        LogERROR("Tcp Connection fd=%d is down ,no more warining \n",channel_->fd());
    }
    LogDEBUG("TcpConnection::handleWrite() ===========> OUT");
}
...
{ 
    ...
    LogERROR("TcpConnection::handleError name:%s - So errno:%d \n",name_.c_str(),err);
}
...
void TcpConnection::send(const std::string & buffer)
{
    ...
    LogDEBUG("state_ == kConnected ? = %s",state_ == kConnected?"Yes":"No");
    if(state_ == kConnected)
    {
        if(loop_->isInLoopThread())
        {
            LogDEBUG("Runing ======runInLoop============> OK");
...
void TcpConnection::sendInLoop(const void * message,size_t len)
{
    ...
    LogDEBUG("IN TcpConnection::sendInLoop ============> OK!!");
    ...
    LogDEBUG("channel_->isWriting() = %s | readableBytes = %ld",channel_->isReading()?"Yes":"NO",outputbuffer_.readableBytes());
    ...
    if(! channel_->isWriting() && outputbuffer_.readableBytes() == 0)
    {
        ...
        if(nwrote >= 0)
        {
            ...
            LogDEBUG("sendOK===============> nwrote=%d , message=%s",(int)nwrote,(char *)message);
        }
        else{ // nwrote < 0 
            nwrote = 0;
        ...
        if(!faultError && remaining > 0)
        {
            ...
            if(!channel_->isWriting())
            {
                channel_->enableWriteEvent(); // 注册channel 的写事件  否则 poller 不会给 channel 通知 epollout
            }
        }
    }
}
...
void TcpConnection::connectEstablelished()
{
    ...
    channel_->enableReadEvent();  // 向poller注册 channel的epollin事件
    // 新连接建立 
